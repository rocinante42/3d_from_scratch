...
update_noise :: proc(value: int) {
    noise_modifier += value
}
...
draw_line_ddr_ditter :: proc(x0, y0, x1, y1: int, color: u32) {
	delta_x := x1 - x0
	delta_y := y1 - y0
    rithm := 2
	...
	for i in 0 ..= longest_side {
        x := int(math.round_f32(current_x))
		if rithm % noise_modifier == 0 {
            draw_pixel(x, int(math.round_f32(current_y)), color) 
        } 
        rithm += 1
...
fill_flat_top_triangle_ditter :: proc(x0, y0, x1, y1, x2, y2: int, color: u32) {
	...
	for y := y2; y >= y0; y -= 1 {
		...
		draw_line_ddr_ditter(x, y, int(x_end), y, color)
		x_start -= inv_slope_a
		x_end -= inv_slope_b
	}
}

fill_flat_bottom_triangle_ditter :: proc(x0, y0, x1, y1, x2, y2: int, color: u32) {
...
fill_flat_top_triangle_stripes :: proc(x0, y0, x1, y1, x2, y2: int, color: u32) {
	inv_slope_a := f32(x2 - x0) / f32(y2 - y0)
	inv_slope_b := f32(x2 - x1) / f32(y2 - y1)
	...
	for y := y2; y >= y0; y -= 1 {
        x := int(math.round(x_start))
		if y % 8 == 0 {draw_line_ddr(x, y, int(x_end), y, color)}
		x_start -= inv_slope_a
		x_end -= inv_slope_b
	}
}

fill_flat_bottom_triangle_stripes :: proc(x0, y0, x1, y1, x2, y2: int, color: u32) {
	inv_slope_a := f32(x1 - x0) / f32(y1 - y0)
	inv_slope_b := f32(x2 - x0) / f32(y2 - y0)
	...
	// loop all the scan lines from top to bottom:
	for y := y0; y <= y2; y += 1 {
		if y % 8 == 0 {draw_line_ddr(int(math.round(x_start)), y, int(x_end), y, color)}
		x_start += inv_slope_a
		x_end += inv_slope_b
	}
}
...
draw_triangle_fill_stripes :: proc(x0, y0, x1, y1, x2, y2: int, fill_color: u32) {
	...
	if y1 == y2 {
		fill_flat_bottom_triangle_stripes(x0, y0, x1, y1, x2, y2, fill_color)
	} else if y0 == y1 {
		fill_flat_top_triangle_stripes(x0, y0, x1, y1, x2, y2, fill_color)
	} else {
		// find Triangle midpoint. 
		my := y1
		mx := f32((x2 - x0) * (y1 - y0)) / f32(y2 - y0) + f32(x0)
		// fill triangle
		fill_flat_bottom_triangle_stripes(x0, y0, x1, y1, int(math.round(mx)), my, fill_color)
		fill_flat_top_triangle_stripes(x1, y1, int(math.round(mx)), my, x2, y2, fill_color)
	}


}

draw_triangle_fill_ditter :: proc(x0, y0, x1, y1, x2, y2: int, fill_color: u32) {
	...
	if y1 == y2 {
		fill_flat_bottom_triangle_ditter(x0, y0, x1, y1, x2, y2, fill_color)
	} else if y0 == y1 {
		fill_flat_top_triangle_ditter(x0, y0, x1, y1, x2, y2, fill_color)
	} else {
		// find Triangle midpoint. 
		my := y1
		mx := f32((x2 - x0) * (y1 - y0)) / f32(y2 - y0) + f32(x0)
		// fill triangle
		fill_flat_bottom_triangle_ditter(x0, y0, x1, y1, int(math.round(mx)), my, fill_color)
		fill_flat_top_triangle_ditter(x1, y1, int(math.round(mx)), my, x2, y2, fill_color)
	}
